...
for input_file in ./genealogos/tests/fixtures/nixtract/flakes/*.in; do
  ...
  echo "Updating: $output_file_1_4"
  GENEALOGOS_DETERMINISTIC=1 genealogos --bom cyclonedx_1.4_json "$flake_ref#$attribute_path" -o "$output_file_1_4"
  echo "Updating: $output_file_1_5"
  GENEALOGOS_DETERMINISTIC=1 genealogos --bom cyclonedx_1.5_json "$flake_ref#$attribute_path" -o "$output_file_1_5"
done

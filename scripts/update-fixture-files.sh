...
for input_file in ./genealogos/tests/fixtures/nixtract/trace-files/*.in; do
  ...
  GENEALOGOS_DETERMINISTIC=1 genealogos --file "$input_file" "$output_file_1_4" --cyclonedx-version 1.4
...
for input_file in ./genealogos/tests/fixtures/nixtract/flakes/*.in; do
  ...
  echo "Updating: $output_file_1_4"
  GENEALOGOS_DETERMINISTIC=1 genealogos --flake-ref "$flake_ref" --attribute-path "$attribute_path" "$output_file_1_4" --cyclonedx-version 1.4
  echo "Updating: $output_file_1_5"
